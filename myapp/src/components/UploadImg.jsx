...
const UploadImg = () => {
    ...
    // using context api
    const fetchFiles = useFetchFiles().fetchImages;
    console.log(useFetchFiles())
    ...
    const handleUploadImage = async (e) => {
        ...
        try {
            const res = await fetch("http://localhost:4001/media/createRequestForImages", {
                method: "POST",
                body: formData,
            ...
            setuploadMessage("Upload");
            toast.success(`${imageFile.name} Uploaded`, {
                position: "top-center",
                autoClose: 3000,
                hideProgressBar: false,
                closeOnClick: true,
    ...
    return (
        <>
            ...
            <div className='mx-auto w-full p-2 text-center border-4 border-[#0ED3CF] h-[340px] rounded-xl'>
                <h3 className='text-center text-[#00928f] font-bold text-xl'>Upload Image</h3>
                <div className='border-dashed rounded-xl border-2 border-[#00928f] text-center mx-auto w-[80%] h-[80%] mt-2 mb-2'>
                    <form method='post'>
                        ...
                        <p className='decoration-1 text-[#00928f]'>(Upload Format PNG, JPG, JPEG)</p>

                        <input type="file" accept='.jpg, .jpeg, .png' onChange={handleFileChange} className="w-full m-2 overflow-hidden text-sm text-center" />

                        {imageFile && <p>{message}</p>}
            ...
            <div id="preview" className="border-2 bg-[#ededed] w-full mt-2 rounded-xl">
                ...
                {fetchFiles && <div className='flex flex-wrap items-center justify-between'>
                    {
                        fetchFiles["images"].map((item, index) => {
                            return (
